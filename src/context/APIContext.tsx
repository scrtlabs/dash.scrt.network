...
const APIContextProvider = ({ children }: any) => {
  ...
  function convertCurrency(
    ...
    const amountInUsd =
      inputAmount / currencyPricing[coinGeckoCurrencyMap[inputCurrency]];

    const convertedAmount =
      amountInUsd * currencyPricing[coinGeckoCurrencyMap[outputCurrency]];
    return convertedAmount;
  }

  const fetchDappsURL = () => {
    fetch(
      "https://raw.githubusercontent.com/SecretFoundation/DappRegistry/main/dAppRegistry.json",
    )
      .then((response) => {
  ...
  useEffect(() => {
      ...
      .then((response) => {
        setDefiLamaApiData_Year(
          response.map((item: { date: string; totalLiquidityUSD: number }) => [
            ...
            currency !== "USD"
              ? convertCurrency("USD", item.totalLiquidityUSD, currency)
    ...
    const COINGECKO_API_URL_MARKET_CAP_VOLUME =
      "https://priceapibuffer.secretsaturn.net/getVolume";
    fetch(COINGECKO_API_URL_MARKET_CAP_VOLUME)
      ...
      .then((response) => {
        ...
        setMarketCap(
          response.secret[coinGeckoCurrencyMap[currency] + "_market_cap"],
        ); // e.g. response.secret.usd_market_cap
    ...
    fetch(API_DATA_SECRET_4)
      ...
      .then((response) => {
        const filteredData = response
          .filter(
            (entry: any) =>
              entry.IBC_Counterpart !== null &&
              entry.IBC_Counterpart !== "secret",
          )
          .map((entry: any) => {
            const chainName =
              bech32PrefixToChainName.get(entry.IBC_Counterpart) ||
              entry.IBC_Counterpart;
            return {
              ...entry,
    ...
    secretjsquery?.query?.distribution
      ...
      ?.then((res) =>
        setCommunityPool(
          Math.floor(
            Number(res.pool.find((entry) => entry.denom === "uscrt").amount) /
              1e6,
          ),
        ),
      );
    ...
    const getBalance = async (address: string) => {
      const res = await secretjsquery?.query?.bank?.balance({
        address,
        denom: "uscrt",
      });
    ...
    secretjsquery?.query?.staking
      .delegatorDelegations({
        delegator_addr: allTokens.find((token) => token.name === "stkd-SCRT")
          .address,
        pagination: { limit: "20" },
      })
