...
export const useSecretNetworkClientStore = create<SecretNetworkClientState>()(
  (set, get) => ({
    ...
    connectWallet: async (walletAPIType?: WalletAPIType) => {
      const { setBalanceMapping, startBalanceRefresh, startIBCBalanceRefresh } =
        get();
      const { walletAddress, secretjs: secretNetworkClient } =
        await WalletService.connectWallet(walletAPIType);
      set({
        walletAPIType,
        walletAddress,
        secretNetworkClient,
    ...
    requestFeeGrant: async () => {
      ...
      const result = await WalletService.requestFeeGrantService(
        feeGrantStatus,
        walletAddress,
      );
    ...
    setIbcBalanceMapping: async (chain: Chain) => {
      ...
      const IbcBalances = await WalletService.fetchIbcChainBalances({
        chain: chain,
    ...
    getIbcBalance(chain: Chain, token: Token) {
      ...
      if (get().ibcBalanceMapping !== null) {
        if (get().ibcBalanceMapping.get(chain) !== undefined) {
          const tokenBalances: TokenBalances = get()
            .ibcBalanceMapping.get(chain)
            .get(token);
          if (tokenBalances) {
            return tokenBalances.balance;
          }
