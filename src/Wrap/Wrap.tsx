...
export function Wrap() {
    ...
    return (
      <>
        ...
        <div className="flex items-center mt-3">
          ...
          <div className="flex-initial text-xs"><PercentagePicker/></div>
        </div>
      </>
    )
  }
  ...
  function WrappedTokenInputComponent() {
    return (
      <>
        ...
        <div className="text-xs mt-3 mb-6"><WrappedTokenBalanceUi/></div>
      </>
    )
  }
  ...
  function SubmitButton(props: { disabled: boolean; nativeAmount: string | undefined; nativeCurrency: string; wrappedAmount: string | undefined; wrappedCurrency: string; wrappingMode: WrappingMode }) {
    const disabled = props.disabled;
    const nativeAmount = props.nativeAmount;
    const nativeCurrency = props.nativeCurrency;
    const wrappedAmount = props.wrappedAmount;
    ...
    return (
      <button
        ...
        {(wrappingMode === WrappingMode.Wrap && nativeAmount) && (<>
          Wrap <span className="text-xs font-normal mx-1">{nativeAmount} {nativeCurrency}</span> into <span className="text-xs font-normal mx-1">{wrappedAmount} {wrappedCurrency}</span>
        </>)}

        {/* text for unwrapping with value */}
        {(wrappingMode === WrappingMode.Unwrap && wrappedAmount) && (<>
          Unwrap <span className="text-xs font-normal mx-1">{wrappedAmount} {wrappedCurrency}</span> into <span className="text-xs font-normal mx-1">{nativeAmount} {nativeCurrency}</span>
        </>)}

        {/* general text without value */}
        {(!nativeAmount || !wrappedAmount) && (<>{wrappingMode === WrappingMode.Wrap ? "Wrap" : "Unwrap"}</>)}
      </button>
    )
  }
    ...
    <>
      <div className="w-full max-w-xl mx-auto px-4">
        <div className="border rounded-lg p-12 pb-7 border-zinc-700 w-full bg-zinc-800 text-zinc-200">
          ...
          <SubmitButton disabled={!selectedToken.address || !nativeValue || !wrappedValue} nativeAmount={nativeValue} nativeCurrency={selectedToken.name} wrappedAmount={wrappedValue} wrappedCurrency={"s" + selectedToken.name} wrappingMode={wrappingMode}/>
