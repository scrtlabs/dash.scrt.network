...
export default function WrapAllTokens() {
  ...
  function getBalanceSpecial(
    token: (typeof tokens)[number],
    isSecretToken: boolean,
  ) {
    const result = getBalance(token, isSecretToken);
    if (result === "viewingKeyError")
      return "viewingKeyError" as GetBalanceError;
    if (result === "GenericFetchError")
  ...
  const filteredTokens = useMemo(() => {
    return tokens.filter((token) => {
      ...
      if (hideZeroBalance && hasViewingKey(token)) {
        ...
        const unwrappedBN =
          unwrapped instanceof BigNumber ? unwrapped : new BigNumber(0);
        const secretBN =
          secret instanceof BigNumber ? secret : new BigNumber(0);
  ...
  const tokensToDisplay = useMemo(() => {
    ...
    const sortedTokens = [...filteredTokens].sort((a, b) => {
      ...
      const balanceA =
        balanceAResult instanceof BigNumber ? balanceAResult : new BigNumber(0);
      const balanceB =
        balanceBResult instanceof BigNumber ? balanceBResult : new BigNumber(0);
    ...
    const selectedTokens = sortedTokens.filter(
      (token) => batchOperations[token.name],
    );
    ...
    setBatchOperations((prev: any) => {
      ...
      if (!prev[tokenName]) {
        ...
        const defaultDirection =
          unwrappedBalance instanceof BigNumber && !unwrappedBalance.isZero()
            ? "wrap"
            : "unwrap";

        // Get the appropriate balance amount based on the chosen direction
        const balanceToUse =
          defaultDirection === "wrap" ? unwrappedBalance : wrappedBalance;
      ...
      // Get the appropriate balance amount based on the chosen direction
      let newAmount =
        newDirection === "wrap" ? unwrappedBalance : wrappedBalance;
  ...
  const handlePerformBatch = async () => {
        ...
        if (token.name === "SCRT" && op.direction === "wrap") {
          ...
          if (rawBalance instanceof BigNumber) {
            const amount = parseFloat(
              rawBalance.dividedBy(`1e${token.decimals}`).toString(),
            );
    ...
    if (!items.length) {
      NotificationService.notify(
        "No valid items selected for batch transaction",
        "error",
      );
      return;
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            <input
              type="checkbox"
              checked={hideNoViewingKey}
              onChange={(e) => setHideNoViewingKey(e.target.checked)}
            />
          ...
          <div className="flex items-center gap-1">
            <input
              type="checkbox"
              checked={showAmountInput}
              onChange={(e) => setShowAmountInput(e.target.checked)}
            />
          ...
          <tbody>
            {isLoading
              ? Array.from({ length: 5 }).map((_, i) => (
                  <tr
                    key={i}
                    className="border-b border-gray-100 dark:border-neutral-600 last:border-0"
                  >
                    <td className="py-3">
                      ...
                      <td className="py-3">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) =>
                              handleChange(token.name, e.target.checked)
                            }
                          ...
                          />
                          <div className="font-semibold justify-center text-base">
                            {token.name}
                          </div>
                          <div>
                            <div className="ml">
                              <button
                                type="button"
                                onClick={() =>
                                  handleCopyAddressToClipboard(token.address)
                                }
                              >
                                <Tooltip
                      ...
                      <td>
                        <BalanceUI
                          ...
                          onBalanceClick={(balanceStr: string) =>
                            handleBalanceClick(token.name, balanceStr, "wrap")
                          }
                        ...
                        <>
                          <td className="text-center">
                            <input
                              ...
                              onChange={(e) =>
                                updateAmount(token.name, e.target.value)
                              }
                              placeholder=""
                              disabled={!isConnected || !token.address}
                            />
