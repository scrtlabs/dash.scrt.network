...
export default function ManageAutoRestakeModal(props: Props) {
  ...
  async function changeRestakeForValidators(
    ...
    try {
      ...
      const txs: MsgSetAutoRestake[] = validatorRestakeStatuses.map(
        (status: ValidatorRestakeStatus) => {
      ...
      await queryTxResult(
        ...
        .catch((error: any) => {
          ...
          if (error?.tx?.rawLog) {
            NotificationService.notify(
              `Setting Auto Restaking failed: ${error.tx.rawLog}`,
              "error",
            );
          } else {
            NotificationService.notify(
              `Setting Auto Restaking failed: ${error.message}`,
              "error",
            );
          }
        })
        .then((tx: any) => {
          if (tx) {
            if (tx.code === 0) {
              NotificationService.notify(
                `Auto Restaking successfully changed`,
                "success",
              );
            } else {
              NotificationService.notify(
                `Changing Auto Restaking failed: ${tx.rawLog}`,
                "error",
              );
            }
          }
