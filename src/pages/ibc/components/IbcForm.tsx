...
import { type Chain, chains, type Token } from "utils/config";
...
export default function IbcForm() {
  ...
  function isValidChainUrlParam(): boolean {
    return !!Object.values(chains).find(
      (chain: Chain) =>
        chain.chain_name.toLowerCase() === chainUrlParam.toLowerCase(),
    );
  }
  ...
  const formik = useFormik<TFormValues>({
    initialValues: {
      chain: selectableChains.find(
        (chain: Chain) => chain.chain_name === "Osmosis",
      ),
  ...
  function setAmountByPercentage(percentage: number) {
    if (formik.values.ibcMode === "withdrawal") {
      ...
      const IbcBalance = getIbcBalance(
        formik.values.chain,
        formik.values.token,
      );
      if (IbcBalance !== null) {
  ...
  useEffect(() => {
    ...
    async function fetchSourceChainAddress() {
      const sourceChainAddr = await IbcService.getChainSecretJs(
        formik.values.chain,
      );
  ...
  return (
    ...
    >
                  ...
                  <img
                    src={
                      ...
                      (formik.values.ibcMode === "deposit"
                        ? formik.values.chain.chain_image
                        : "scrt.svg")
                    }
      ...
      {formik.values.token.is_axelar_asset &&
        formik.values.chain.chain_name != "Axelar" && (
          <BridgingFees
