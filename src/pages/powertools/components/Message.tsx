...
function Message(props: Props) {
  ...
  useEffect(() => {
    (async () => {
      if (MessageDefinitions[props.type]?.relevantInfo && props.secretjs) {
        ...
        setRelevantInfo(
          await MessageDefinitions[props.type].relevantInfo!(
            props.secretjs,
            props.prefix,
            props.denom,
            props.content,
  ...
  const selectOptions: { value: any; label: string }[] = Object.keys(
    MessageDefinitions,
  )
    .sort((a, b) => {
      const module = MessageDefinitions[a].module.localeCompare(
        MessageDefinitions[b].module,
      );
      if (module !== 0) {
        return module;
      } else {
  ...
  const onTypeSelect = (messageType: any) => {
    ...
    const newContent: string = JSON.stringify(
      MessageDefinitions[messageType].example(
        props.secretjs,
        null,
        props.prefix,
        props.denom,
    ...
    <div>
      <div className="inline-flex items-center justify-center w-full">
        ...
        <span className="inline-flex items-center gap-2 select-none absolute px-3 text-black -translate-x-1/2 bg-white left-1/2 dark:text-white dark:bg-neutral-900 text-sm font-bold">
          ...
          <Tooltip
            title={
              props.disabled ? "Cannot delete when disabled" : "Delete Message"
            }
          >
      ...
      <div>
        ...
        <div className="my-4">
          <textarea
            value={props.content}
            onChange={(e) =>
              props.disabled ? null : props.updateContent(e.target.value)
            }
            onBlur={handleBlur}
