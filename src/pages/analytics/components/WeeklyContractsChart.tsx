  ...
  useEffect(() => {
    if (analyticsData5) {
      ...
      analyticsData5.forEach((item: Data) => {
        ...
        dataByDateAndContract[item.Date][item.contract_name] +=
        ...
        totalTransactionsPerContract[item.contract_name] +=
          item.num_transactions;
      });
      ...
      const sortedContractNames = contractNames.sort(
        (a, b) =>
          totalTransactionsPerContract[b] - totalTransactionsPerContract[a],
      );
      const topContractNames = sortedContractNames.slice(0, topN);

      // Create datasets for top contracts
      const datasets: ChartDataset<"bar", number[]>[] = topContractNames.map(
        (contractName, index) => {
          const data = dates.map(
            (date) => dataByDateAndContract[date]?.[contractName] || 0,
          );
          return {
            label: contractName,
            data,
      ...
      <div>
        <h2 className="text-center text-xl font-semibold pt-2.5 pb-0">
          ...
          <div className="inline-block">
            <Tooltip
              title="Number of transactions per contract per week"
              placement="right"
              arrow
            >
