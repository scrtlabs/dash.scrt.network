...
export default function UnbondingsChart() {
  ...
  useEffect(() => {
    if (L5AnalyticsApiData) {
      const dataArray = Object.entries(
        L5AnalyticsApiData["unbonding_by_date"],
  ...
  return (
    <>
      <div>
        <h2 className="text-center text-xl font-semibold pt-2.5 pb-0">
          ...
          <div className="inline-block">
            <Tooltip
              title={`Shows the unbonded (unstaked) SCRT per day`}
              placement="right"
              arrow
            >
      ...
      {totalUnbonding ? (
        <h3 className="text-center text-lg pt-0 pb-2.5">
          Total: {Math.round(totalUnbonding).toLocaleString()} SCRT
        </h3>
      ) : null}
      ...
      <div className="text-center text-sm">
        Metrics by Lavender.Five Nodes 🐝
      </div>
    </>
  );
}
