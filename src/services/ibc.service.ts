...
async function getChainSecretJs(chain: Chain): Promise<SecretNetworkClient> {
  ...
  const sourceOfflineSigner =
    await window.wallet.getOfflineSignerAuto(chain_id);
...
async function performIbcDeposit(
  ...
  const deposit: Deposit = token.deposits.filter(
    (deposit: Deposit) => deposit.chain_name === props.chain.chain_name,
  )[0];
  ...
  try {
    if (
      !["Evmos", "Injective", "Dymension"].includes(props.chain.chain_name) &&
      !token.is_axelar_asset
    ) {
      ...
      if (token.name === "ampLUNA" || token.name === "bLUNA") {
        ...
        const contractMsg = new (Terra as any).MsgExecuteContract(
          ...
          token.deposits
            .find(
              (deposit: Deposit) =>
                deposit.chain_name === props.chain.chain_name,
            )
            .denom.substring("cw20:".length),
          {
            send: {
              ...
              timeout: 900,
              contract:
                "terra1e0mrzy8077druuu42vs0hu7ugguade0cj65dgtauyaw4gsl4kv0qtdf2au", //cw20-ics20 contract on Terra
        ...
        const terraSigner = await window.wallet.getOfflineSignerAuto(
          props.chain.chain_id,
        );
        ...
        const { account } = await (
          await fetch(
            `${selectedSource.lcd}/cosmos/auth/v1beta1/accounts/${sourceChainNetworkClient.address}`,
          )
        ).json();
        ...
        const routing = useSKIPRouting
          ? await getSkipIBCRouting(
              ...
              "deposit",
              token,
              new BigNumber(props.amount).multipliedBy(
        ...
        const receiver = useSKIPRouting
          ? await getReceiverAddress(
              (routing.operations[1] as any).transfer.chainID,
            )
          : props.secretNetworkClient.address;
          ...
          [
            {
              value: {
                ...
                sourcePort: useSKIPRouting
                  ? (routing.operations[0] as any).transfer.port
                  : "transfer",
                sourceChannel: useSKIPRouting
                  ? (routing.operations[0] as any).transfer.channel
                  : deposit_channel_id,
                receiver: receiver,
              },
      ...
      const evmosProtoSigner = await window.getOfflineSignerAuto!(
        selectedSource.chain_id,
      );
    ...
    tx = await queryTxResult(
      sourceChainNetworkClient,
      tx.transactionHash,
      5000,
      10,
    );
  ...
  try {
    if (token.is_snip20) {
      ...
      const transferMsg = new MsgTransfer({
        sender: props.secretNetworkClient?.address,
        receiver: receiver,
        source_channel: useSKIPRouting
          ? (routing.operations[0] as any).transfer.channel
          : withdraw_channel_id,
        source_port: useSKIPRouting
          ? (routing.operations[0] as any).transfer.port
          : "transfer",
        token: {
          amount,
    ...
    if (tx.code !== 0) {
      ...
      NotificationService.notify(
        `Sent ${props.amount} ${token.name} to ${selectedDest.chain_name}`,
        "success",
        toastId,
      );
    }
...
async function fetchSourceBalance(
  ...
  chain: Chain,
  token: Token,
): Promise<BigNumber | GetBalanceError> {
  if (!address) {
...
async function getSkipIBCRouting(
  chain: Chain,
  IbcMode: IbcMode,
  token: Token,
  amount: BigNumber,
  ...
  let source;
  if (IbcMode === "deposit") {
    source = token.deposits.find(
      (deposit) => deposit.chain_name === chain.chain_name,
    );
    dest = token.withdrawals.find(
      (withdrawals) => withdrawals.chain_name === chain.chain_name,
    );
  }
  if (IbcMode === "withdrawal") {
    dest = token.deposits.find(
      (deposit) => deposit.chain_name === chain.chain_name,
    );
...
function composePMFMemo(
  ...
  async function generateMemo(
    ...
    const nextMemo: string = await generateMemo(
      operations.slice(1),
      finalNextContent,
    );
    ...
    if (operations.length === 1) {
      ...
      receiver = await getReceiverAddress(
        (operations[1] as any).transfer.chainID,
      );
    }

    // If this is the last operation and there are no more operations, use finalNextContent as the next memo
    const next =
      operations.length > 1 ? JSON.parse(nextMemo) : finalNextContent;
...
function getSupportedChains(): Chain[] {
  const selectableChains: Chain[] = Object.values(chains).filter(
    (chain) => chain.chain_name !== "Secret Network",
  );
 ...
 */
function getSupportedChainByIbcTokenName(
  tokenName: string,
): string | undefined {
  ...
  const supportedDeposit = token.deposits.find((deposit) =>
    allTokens.some((t) =>
      t.deposits.some((d) => d.chain_name === deposit.chain_name),
    ),
  );
...
async function performIbcTransfer(props: TProps): Promise<string> {
  const sourceChainNetworkClient = await IbcService.getChainSecretJs(
    props.chain,
  );
