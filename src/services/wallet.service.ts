...
const requestFeeGrantService = async (
  ...
  if (feeGrantStatus === "success") {
    ...
    try {
      const result = await (
        await fetch(`${faucetURL}/${walletAddress}`)
      ).json();
      if (result?.feegrant) {
        newFeeGrantStatus = "success";
        NotificationService.notify(
          `Successfully sent new fee grant (0.05 SCRT) to address ${walletAddress}`,
          "success",
        );
      } else {
      ...
      newFeeGrantStatus = "fail";
      NotificationService.notify(
        `Fee Grant for address ${walletAddress} failed with error: ${error}`,
        "error",
      );
    }
  }
...
async function fetchIbcChainBalances(
  ...
  const { balances }: QueryAllBalancesResponse =
    await sourceChain.query.bank.allBalances({
  ...
  allTokens.forEach((token) => {
    const fromDenom = token.deposits.find(
      (deposit) => deposit.chain_name === props.chain.chain_name,
    )?.denom;
    if (fromDenom) {
  ...
  tokensToCheck.forEach(async (tokenName) => {
    ...
    if (token && props.chain.chain_name === "Terra") {
      const tokenDeposits = token.deposits.filter(
...
async function getBalancesForTokens(
  props: IGetBalancesForTokensProps,
): Promise<Nullable<Map<Token, TokenBalances>>> {
  ...
  try {
    const { balances }: QueryAllBalancesResponse =
      await props.secretNetworkClient.query.bank.allBalances({
    ...
    const secretBalances = await getBatchsTokenBalance(
      ...
      if (currentEntry) {
        if (
          ...
          newBalanceMapping.set(token, {
            ...currentEntry,
            secretBalance:
              secretBalance !== null ? new BigNumber(secretBalance) : null,
          });
        }
      } else {
        if (
